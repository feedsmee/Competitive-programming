...
struct WaveTree {
  ...
  // in S.
  WaveTree(vector<int> &A, int sigma) : C(sigma * 2), s(sigma) { build(A.begin(), A.end(), 0, s - 1, 1); }
  ...
  void build(iter b, iter e, int L, int U, int u) {
    ...
    C[u].reserve(e - b + 1);
    C[u].push_back(0);
    for (iter it = b; it != e; ++it) C[u].push_back(C[u].back() + (*it <= M));
